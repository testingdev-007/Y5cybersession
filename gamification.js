// ============================================================
// GAMIFICATION.JS — CyberShield Academy
// Persists across resets (memory only) — clears on page reload
// ============================================================

// ── STATE — survives resetAll() ──────────────────────────────
const GAMIFICATION = {
  sessions:  [],
  trophySet: new Set(),
  totalRuns: 0,
...
const TROPHIES = {
  ...
  quiz_ace:      { name:'Quiz Ace',         icon:'🧠', desc:'All quiz questions right in one run' },
...
function showEndSplash(){
  ...
  const sess = {
    ...
    phishReported:GS.phishReported||false, ipWon:GS.ipWon||false,
    rounds:GS.round,
  };
  ...
  let cur=0; const step=Math.max(1,Math.ceil(finalXP/60));
  ...
  if(newTrophies.length){
    ntEl.innerHTML='<div class="es-nt-lbl">UNLOCKED THIS RUN!</div><div class="es-nt-row">'+
      newTrophies.map(id=>`<div class="es-nt-badge">${TROPHIES[id].icon}<div>${TROPHIES[id].name}</div></div>`).join('')+'</div>';
...
function _buildResults(sess,grade){
  document.getElementById('esCont_results').innerHTML=`
    ...
    <div class="es-break">
      ...
      ${sess.quizCorrect>0?`<div class="es-brow"><span>Quiz bonuses (${sess.quizCorrect} × 15 XP)</span><span>+${sess.quizCorrect*15}</span></div>`:''}
...
// ── BUILD: RUN HISTORY ────────────────────────────────────────
function _buildRuns(){
  const sess=GAMIFICATION.sessions;
  if(!sess.length){document.getElementById('esCont_runs').innerHTML='<div class="es-empty">No history yet!</div>';return;}
  const sorted=[...sess].sort((a,b)=>b.finalXP-a.finalXP);
  const cur=GAMIFICATION.totalRuns;
  document.getElementById('esCont_runs').innerHTML=
    ...
    sorted.map((s,i)=>`
      <div class="es-rrow ${s.run===cur?'cur':''} ${i===0?'best':''}">
